class  VariableQuiz
{
	...
	VariableQuiz(double c){
		...
		System.out.println(c); // 파라미터로 디폴트 값인 0.0이 출력됨
		System.out.println(d); // 지역변수에 값할당이 안된 변수를 출력하면 에러
		System.out.println(e); // 지역변수나 클래스에 선언되지 않은 변수를 출력 하여 에러
		System.out.println(f); // 다른 지역변수인 값을 출력하여 에러
		System.out.println(g); // 28번 라인에 선언된 클래스 변수를 호출함 디폴트 값인 0이 출력
	}
	...
	void writeVariable(int e){
		...
		System.out.println(a); // 객체 변수로 디폴트인 null이 출력됨
		System.out.println(b); // 클래스 변수로 디폴트인 null이 출력됨
		System.out.println(c); // 지역이나 클래스내에 해당 변수가 선언된것이 없어 출력되지 않음
		System.out.println(d); // 지역이나 클래스내에 해당 변수가 선언된것이 없어 출력되지 않음
		System.out.println(e); // 파라미터에 선언된 e의 디폴트 값인 0이 출력됨
